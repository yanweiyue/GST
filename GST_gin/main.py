    ...
    for epoch in range(args['total_epoch']+args['pretrain_epoch']+args['retrain_epoch']):
        ...
        if epoch == args['pretrain_epoch']:
            ...
            if args["remain"] is not None:
                ...
                pruner = GSTScheduler(net_gcn, optimizer, remain=args["remain"], alpha=args["alpha"], delta=args["delta"], static_topo=args["static_topo"],
                                       T_end=T_end,  accumulation_n=args["accumulation_n"],ignore_linear_layers=not args["spar_wei"],
                                   pretrain=args['pretrain_epoch'],ignore_parameters=not args['spar_adj'],beta=args['beta'],warmup_steps=args["warmup_steps"],
                                   topo_k=args['topo_k'],sema_k=args['sema_k'],dataset=args["dataset"],backbone='gin')
        ...
        add_loss = 0
        add_loss += utils.log_variance_loss(net_gcn.edge_score)*0.01 # Chameleon 0.02
        loss += add_loss
        loss.backward()
        optimizer.step()
