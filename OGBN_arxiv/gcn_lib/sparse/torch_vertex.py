...
class GENConv(GenMessagePassing):
    ...
    def __init__(self, in_dim, emb_dim,
                 ...
                 eps=1e-7, edge_num=2484941,spar_adj=False):
    ...
    def forward(self, x, adj_mask1_train,  edge_index, edge_attr=None):
        x = x
        if self.spar_adj:
            if self.adj_mask1_train == None:
                self.adj_mask1_train = adj_mask1_train
            else:
                self.adj_mask1_train.data = adj_mask1_train
    ...
    def aggregate(self, inputs, index, ptr=None, dim_size=None):
        ...
        if self.aggr in ['add', 'mean', 'max', None]:
            ...
            inputs = inputs * self.adj_mask1_train
            if self.learn_t:
